...
// Android/simple-node 环境没有 electron，此模块的 BrowserWindow 登录窗口功能将降级不可用。
// （qishui 扫码登录依赖 Electron 的 BrowserWindow + session，纯 Node 服务仅保留错误提示。）
...
function assertElectronAvailable() {
  if (!BrowserWindow || !app || !session) {
    const error = new Error('QISHUI_AUTH_ELECTRON_UNAVAILABLE: qishui 扫码登录依赖 Electron BrowserWindow/session，当前纯 Node 环境不可用');
    error.code = 'QISHUI_AUTH_ELECTRON_UNAVAILABLE';
    throw error;
  }
}
